...
## Provider-Saving Dynamic ports of remote server and Private Key
 def provider_portsave():
   ...
   fileopen = open(DIRECTORY + '/' + PRIVATE_KEY_FILE, 'w')
   print ( PRIVATE_KEY, file=fileopen)
   os.chmod(DIRECTORY + '/' + PRIVATE_KEY_FILE, 0o600)
...
## Connection Establishment
 if CON_OPTION == "P":
   ...
   cmd = "ssh -fN -R {REMOTE_PORT}:localhost:{LOCAL_PORT} {USERNAME}@{REMOTE_IP} -i {DIRECTORY}/{PRIVATE_KEY_FILE} 2> {DIRECTORY}/{LOCAL_PORT}.txt"
